...
pub async fn get_optifine_version_names(mc_version: String) -> Result<Vec<String>, String> {
    ...
    let version_re =
        Regex::new(&version_pattern).map_err(|e| format!("构建正则表达式失败: {}", e))?;
    ...
    if let Some(version_pos) = version_re.find(&html) {
        ...
        let next_version_re = Regex::new(r#"<h2>Minecraft \d+\.\d+\.\d+</h2>"#)
            .map_err(|e| format!("构建正则表达式失败: {}", e))?;
        let end_pos =
            if let Some(next_match) = next_version_re.find(&content[version_pattern.len()..]) {
...
pub async fn install_optifine(
    ...
    let opt_wrapper_path = config_dir
        .join("optWrapper.jar")
        .to_string_lossy()
        .to_string();
